...
DGX Spark에서 본인 계정(<userA> / <userB> / <userC>)으로 dev 컨테이너를 띄우는 가장 짧은 절차. 호스트 docker 그룹 가입 없이 본인 전용 rootless dockerd로 동작.
...
```bash
ssh <userA>@<HOST_IP>        # 본인 계정명으로
```

**중요** — 반드시 직접 SSH로 들어와야 함. `su - <userA>` 같은 방식은 systemd --user 세션이 없어서 다음 단계가 깨짐.
...
SSH_PORT_LOCAL=2223
SSH_PORT_TS=2223
TAILSCALE_IP=<HOST_IP>
LAN_IP=<LAN_IP>
```

본인 UID 확인 — `id -u`. 본인 GID — `id -g`. 둘 다 같은 값.

<userB>이면 1002, 포트 2224. <userC>이면 1003, 포트 2225. 나머지는 동일.
