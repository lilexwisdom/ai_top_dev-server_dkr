<!-- (아카이브) 자동화 스크립트가 도입되기 전 손-셋업 가이드. 디버깅·재구성 시 참조용으로 보존. -->
# (아카이브) Manual Setup — 손으로 따라하는 사용자 셋업

> **이 문서는 아카이브.** 자동화 스크립트(`scripts/admin/add-user.sh` + `scripts/user/setup.sh`)가 동일 절차를 멱등으로 수행. 새 사용자 셋업은 [`Quick_setup.md`](../Quick_setup.md) 참조. 이 문서는 자동화가 깨졌을 때 손으로 따라가거나 단계별 동작을 이해하기 위해 보존.
