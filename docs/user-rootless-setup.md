...
> 이 문서를 읽고 있는 너는 Claude Code다. 사용자(<userA> / <userB> / <userC> 중 한 명)의 계정에서 본인 전용 dev 컨테이너를 띄우는 작업을 한다.
...
**SSH 포트 할당** — 한 머신에서 다중 사용자가 동시 운영하므로 충돌 회피용으로 사용자별 고정.
...
본인이 위 표에 없으면 멈추고 사용자에게 어느 포트를 쓸지 물어봄.
...
### 포트 충돌 — `Bind for <HOST_IP>:2223 failed: port is already allocated`
